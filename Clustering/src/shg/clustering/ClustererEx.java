...
public class ClustererEx {
	...
	DataPackage input_data = null;
	
	//list of clusters
	private LinkedList<ClusterEx> clusters = null;
	//distance calculator
	...
	//minimum distance
	MinDistanceEx min = new MinDistanceEx();
	//minimum distances - temp memory for calculation
	LinkedList<MinDistanceEx> min_dists = null;
	 ...
	 */
	public List<ClusterEx> getClusters() {
		return clusters;
	}
	...
	private void _create_list_of_clusters(){
		clusters = new LinkedList<ClusterEx>();
		ClusterEx cluster = null;
		for (DataVector d : input_data.getList()) {
			cluster = new ClusterEx();
			cluster.addVector(d);
	...
	private void _create_and_init_distances() {
		...
		min_dists = new LinkedList<MinDistanceEx>();
		min.distance = -1.0;
		MinDistanceEx tmp_min = new MinDistanceEx();
		ClusterEx one, two;
		boolean first;
		double tmp;
		int i=1, j;
		...
		ListIterator<ClusterEx> it_one = clusters.listIterator();
		...
		while (it_one.hasNext()){
			...
			ListIterator<ClusterEx> it_two = clusters.listIterator();
			...
			i++;
			dists.add(dists_row);
			min_dists.add(new MinDistanceEx(tmp_min));
			if (min.distance == -1.0) {
				min.copy(tmp_min);
			} else {
	...
	private void _cluster() {
		...
		MinDistanceEx tmp;
		double tmp_double;
		ClusterEx one,two,to_delete;
		int idx, column, row;
		List<Double> tmp_dist_list, dist_list=null;
		ListIterator<ClusterEx> cl_it;
...
// ^- for tests only			
			...
			ListIterator<MinDistanceEx> mins_it = min_dists.listIterator(idx);
			if (list_it.hasNext()){
				list_it.next();
...
//			printDistances();
			...
			if (column <= dists.size()) {
				...
				ListIterator<MinDistanceEx> min_dist_it = min_dists.listIterator(column);
				MinDistanceEx tmp_min_dist;
...
//			printDistances();
			...
			column = 0;
			row = 1;
			ListIterator<ClusterEx> cl_tmp_a = clusters.listIterator();
			ListIterator<ClusterEx> cl_tmp_b;
			ClusterEx tmp_cluster;
			list_it = dists.listIterator();

			one = cl_tmp_a.next(); //skip first (0) row
			
			first = true;
			for (MinDistanceEx elem : min_dists) {
				tmp = elem;
				one = cl_tmp_a.next();
				dist_list = list_it.next();
